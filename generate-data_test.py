...
def gather_datum(prog, path, base, additional_flags, timeout):
    ...
    t1 = time.time()
    return (t1-t0,process_output.stdout,process_output.stderr)
...
def gather_data(rootlength, prog, path, base):
    ...
    def gather_col(flags, run_combiner, col_name, timeout_time, repetition_count):
        ...
        if error:
            ...
            current_data[col_name]=run_combiner(run_data,time_data)

    def ctime_combiner(run_data_transpose,time_data):
        computation_time_col = [float(x) for x in time_data]
    ...
    gather_col([],ctime_combiner,"RunTime",TIMEOUT_TIME,REPETITION_COUNT)
...
def print_data(data):
    ...
    with open("generated_data/data_test.csv", "wb") as csvfile:
