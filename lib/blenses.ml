	...
	: (L.lens * L.regex * L.regex) option * Lenscontext.LensContext.t =
		...
		if not (bijective l) then None, lc else
			...
			(L.lens * L.regex * L.regex * bool) * Lenscontext.LensContext.t =
				match l.desc with
				| Var (s, ml) ->
					let (l, r1, r2, b), lc = helper i ml lc in if not b then dummy, lc else
						(L.LensVariable s, r1, r2, true), lc
						(*begin
							...
							| None ->
									let (l, r1, r2, b), lc = helper i ml lc in if not b then dummy, lc else
										begin
											let lc =
												match Lenscontext.LensContext.lookup lc s with
												| None -> Lenscontext.LensContext.insert_exn lc s l r1 r2
												| Some (l, r1, r2) -> lc in
											(L.LensVariable s, r1, r2, true), lc
										end
							| Some (l, r1, r2) -> (L.LensVariable s, r1, r2, true), lc
						end*)
				| Copy r -> let r, _ = Brx.brxToLrx r i rc in (L.LensIdentity r, r, r, true), lc
				| Invert ml ->
				...
				| Concat (ml1, ml2) ->
						...
						(L.LensConcat (ml1, ml2), L.RegExConcat(s1, s2), L.RegExConcat(v1, v2), b1 && b2), lc
				| Union (ml1, ml2) ->
						let (ml1, s1, v1, b1), lc = helper i ml1 lc in
						let (ml2, s2, v2, b2), lc = helper i ml2 lc in
						(L.LensUnion (ml1, ml2), L.RegExOr(s1, s2), L.RegExOr(v1, v2), b1 && b2), lc
				| Star ml1 ->
						let (ml1, s1, v1, b1), lc = helper i ml1 lc in
			...
			let (l, r1, r2, b), lc = helper i l lc in if not b then None, lc else
				Some (l, r1, r2) , lc
