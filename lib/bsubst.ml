...
and free_svars_exp acc = function
	| ESynth(_, e1, e2, e3) ->
			...
			let acc2 = free_svars_exp acc1 e2 in
			let acc3 = match e3 with
				| None -> acc2
				| Some l -> List.fold_left (fun acc3 e -> free_svars_exp acc3 e) acc2 l in
			acc3
	| EPerm(_, l, e2) ->
			let acc1 = List.fold_left (fun acc1 e -> free_svars_exp acc1 e) acc l in
			let acc2 = free_svars_exp acc1 e2 in
			acc2
	| EProject(_, e1, e2) ->
			let acc1 = free_svars_exp acc e1 in
...
(* TODO: optimize all of these so we don't cons up new ASTs unless they've *)
(* actually changed!                                                       *)
...
and subst_svars_sort subst s0 = match s0 with
	...
	| SData(sl, qx) ->
			...
			SData(new_sl, qx)
	| SRefine(x, b0, s1, e2, k) ->
			let new_s1 = subst_svars_sort subst s1 in
			let new_e2 = subst_svars_exp subst e2 in
			SRefine(x, b0, new_s1, new_e2, k)
	| SForall(a, s1) ->
...
and subst_svars_exp subst e0 = match e0 with
	| ESynth(i, e1, e2, l) ->
			...
			let new_e2 = subst_svars_exp subst e2 in
			let l = Util.get_option_list l in
			let new_l = List.fold_left (fun l e -> (subst_svars_exp subst e) :: l) [] l in
			ESynth(i, new_e1, new_e2, Some (List.rev new_l))
	| EProject(i, e1, e2) ->
			let new_e1 = subst_svars_exp subst e1 in
			let new_e2 = subst_svars_exp subst e2 in
...
and free_evars_exp acc = function
	| ESynth(_, e1, e2, l) ->
			...
			let acc3 = match l with
				| None -> acc2
				| Some l -> List.fold_left (fun acc3 e -> free_evars_exp acc3 e) acc2 l in
			acc3
	| EProject(_, e1, e2) ->
			let acc1 = free_evars_exp acc e1 in
			let acc2 = free_evars_exp acc1 e2 in
			acc2
	| EPerm(_, l, e2) ->
			let acc1 = List.fold_left (fun acc1 e -> free_evars_exp acc1 e) acc l in
			let acc2 = free_evars_exp acc1 e2 in
			acc2
	| EVar(_, q) ->
			Qid.Set.add q acc
	...
	| ELet(_, Bind(_, p1, so2, e3), e4) ->
			...
			(* calculate bvars --- we don't want to count occurrences in the let *)
			(* body as free vars                                                 *)
...
(* msg "@[SUBST: {"; Misc.format_list ",@ " (fun (x,e) -> msg "@[%s |->    *)
(* %s@]" (Qid.string_of_t x) (string_of_exp e)) subst; msg "} IN           *)
(* %s@]@\n@[X: %s < %b > FRESH_X: %s@]@\n" (string_of_sort s0)             *)
(* (Id.string_of_t x) (x = fresh_x) (Id.string_of_t fresh_x);              *)
...
and subst_evars_sort subst s0 = match s0 with
	...
	| SRefine(x, b0, s1, e2, k) ->
			...
			SRefine(fresh_x, b0, new_s1, new_e2, k)
	| SForall(a, s1) ->
			let new_s1 = subst_evars_sort subst s1 in
			SForall(a, new_s1)
	| SUnit | SBool | SInteger | SChar | SString | SRegexp | SAregexp | SSkeletons | SResources | SLens | SCanonizer | SVar _ | SPrefs _ ->
...
and subst_evars_exp subst e0 = match e0 with
	...
	| ESynth(i, e1, e2, l) ->
			...
			let new_e2 = subst_evars_exp subst e2 in
			let l = Util.get_option_list l in
			let new_l = List.fold_left (fun l e -> (subst_evars_exp subst e) :: l) [] l in
			ESynth(i, new_e1, new_e2, Some (List.rev new_l))
	| EProject(i, e1, e2) ->
			let new_e1 = subst_evars_exp subst e1 in
			let new_e2 = subst_evars_exp subst e2 in
...
let rec expose_sort = function
	| SRefine(x, b0, s1, e1, _) -> expose_sort s1
	| s0 -> s0
...
let rec syneq_sort s1 s2 = match s1, s2 with
	...
	| SRefine(x, b1, s1, e1, _), SRefine(y, b2, s2, e2, _) ->
...
and qualify_exp resolve bound e0 = match e0 with
	...
	| ESynth(i, e1, e2, l) ->
			...
			let l = Util.get_option_list l in
			let new_l = List.fold_left (fun l e -> (qualify_exp resolve bound e) :: l) [] l in
			ESynth(i, new_e1, new_e2, Some (List.rev new_l))
	| EApp(i, e1, e2) ->
			let new_e1 = qualify_exp resolve bound e1 in
			let new_e2 = qualify_exp resolve bound e2 in
