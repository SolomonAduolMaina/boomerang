...
(* --------------------- PRETTY PRINTING --------------------- ranks: used *)
(* in formatting to decide when parentheses are needed.                    *)
type r =
	| Urnk (* union *)
	| Drnk (* diff *)
	| Irnk (* inter *)
...
(* a printable version of the representative (do not use internally unless *)
(* it is to print something)                                               *)
...
let rec size
  ...
  begin match r.desc with
		| CSet(cs) -> (CharSet.size cs, processed_variables)
	  | Seq (r1,r2) ->
      let (s1,processed_variables) = size r1 processed_variables in
      let (s2,processed_variables) = size r2 processed_variables in
      (s1+s2+1,processed_variables)
	  | Alt l ->
      List.fold_left
        (fun (size_acc,processed_variables) r ->
          let (sr,processed_variables) =
            size
              r
              processed_variables
          in
          (size_acc + sr + 1, processed_variables))
       (-1,processed_variables)
       l
	  | Rep (r,_,_) ->
      let (size,processed_variables) = size r processed_variables in
      (size+1,processed_variables)
		| Inter l ->
      List.fold_left
        (fun (size_acc,processed_variables) r ->
          let (sr,processed_variables) =
	  ...
	  | Var (s,r') ->
       if s = "_" || s = "cn" || s = "cn1" || s = "cn2" || s = "r" || s = "r1" || s = "r2" then
         ...
         (if (List.mem s processed_variables) then
           ...
           let (s,processed_variables) = (size r' processed_variables) in
           (1+s,processed_variables))
	  | Perm (rs,r') ->
      let (s,processed_variables) = size r' processed_variables in
      List.fold_left
        (fun (size_acc,processed_variables) r ->
           let (sr,processed_variables) = size r processed_variables in
           (size_acc+sr,processed_variables))
